...
const createRole = async (req, res) => {
  ...
  try {
      const existingRole = await sql.query`SELECT * FROM Roles WHERE RoleName = ${roleName}`;
...
const updateRole = async (req, res) => {
  ...
  try {
    const result = await sql.query`UPDATE Roles SET RoleName = ${roleName} WHERE RoleID = ${id}`;
